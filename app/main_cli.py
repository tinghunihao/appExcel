...
def write_crash_log() -> None:
    message = traceback.format_exc()
    try:
        Path("compat_error.log").write_text(message, encoding="utf-8")
    except Exception:
        pass
    print("\n程序启动或执行失败，错误如下：")
    print(message)
    print("如果当前目录生成了 compat_error.log，请把这个文件发给开发人员。")
...
def load_logic():
    ...
    return {
        ...
        "complete_address_workbook": complete_address_workbook,
        "complete_anhui_address": complete_anhui_address,
        "extract_and_merge_workbook": extract_and_merge_workbook,
        "match_order_workbooks": match_order_workbooks,
    }
...
def run_address_preview() -> None:
    ...
    completed, status = logic["complete_anhui_address"](address)
    print("\n补全结果：")
    print(completed)
    print(f"状态：{status}")
...
def run_address_batch() -> None:
    ...
    stats = logic["complete_address_workbook"](input_path, output_path, address_column)
    print(f"\n处理完成：{output_path}")
...
def run_extract_merge() -> None:
    ...
    stats = logic["extract_and_merge_workbook"](
        raw_path,
        master_path,
        output_path,
        selected_columns,
...
def run_ab_match() -> None:
    ...
    stats = logic["match_order_workbooks"](a_path, b_path, output_path, columns)
    print(f"\n处理完成：{output_path}")
